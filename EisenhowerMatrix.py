...
with st.form("task_form"):
    task = st.text_input("タスクを入力してください", st.session_state["task_input"])
...
if submitted and task:
    new_task = pd.DataFrame([[task, st.session_state["importance"], st.session_state["urgency"]]], columns=["タスク", "重要度", "緊急度"])
    st.session_state["tasks"] = pd.concat([st.session_state["tasks"], new_task], ignore_index=True)
    st.session_state["task_input"] = ""  # 入力フィールドをリセット
...
with col2:
    st.markdown('<divの="column2">', unsafe_allow_html=True)
